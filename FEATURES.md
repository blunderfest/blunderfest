...
- ✅ Whole-game visualization (ADR-0024 as amended): the **timeline band** under the board — Eval | Material | Activity | Clocks as stacked, toggleable layers on one shared move axis (persisted per viewer); eval chart with blunder dots, cp/win% toggle, opening book-exit marker, opening/endgame phase shading, capture/exchange marks; material and piece-activity timelines (pure FEN data, no engine); thinking-time bars from `[%clk]` data (extracted at parse time; Lichess imports fetch clocks). The sidebar viz box keeps the list views: Moments (mini boards of the biggest swings) and Report
