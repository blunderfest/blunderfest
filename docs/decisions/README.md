...
| [0024](adr-0024-reference-tab-and-search-destination.md) | Feature docking: per-position reference in an adaptive Reference tab; whole-game views in the viz box; search is a `#/search` destination | Accepted (2026-08-17); docking contract amended by ADR-0031, Reference-tab home by ADR-0032 |
...
| [0031](adr-0031-one-sidebar-room-layout.md) | One-sidebar room layout: presence as app-bar chrome, Share in the header, band as a strip, mobile tabbed sheet, anti-clutter docking contract | Accepted (2026-08-27); region structure & Room-tab scope superseded by ADR-0032 |
...
| [0036](adr-0036-corpus-resourced-to-broadcast-database.md) | Corpus re-sourced to the Lichess Broadcast Database (~1.17M elite OTB games); drop non-standard games, no Elo floor; extraction variant/FEN-correct | Accepted (2026-08-30) |
| [0037](adr-0037-packed-occurrence-backend.md) | Packed binary occurrence backend behind the `Corpus` facade (`occurrence_backend` config): parity exact, product parity correct, broadcast 1.17M validated; PG stays for games/moves/metadata | Accepted (2026-08-31); broadcast validated (2026-09-02) |
| [0038](adr-0038-packed-format-v2-position-stats.md) | Packed format v2: pos.bin headers carry pack-time run statistics (`occurrence_count`, `game_count`, `occ_run_offset`), manifest v2, built behind a flag with sampled re-count validation; product/API cutover pending | Accepted (2026-09-05) — implementation behind flag |
| [0039](adr-0039-he-hydration-batched-topology-stays-single-region.md) | HE card hydration batched behind the Corpus boundary (bulk `games(gids)` + `moves_for` reuse; 45 → 2–3 round trips); the ~10 s cross-region penalty was the request shape, so the single ams PostgreSQL + ams/ord topology stays | Accepted (2026-09-05) |
