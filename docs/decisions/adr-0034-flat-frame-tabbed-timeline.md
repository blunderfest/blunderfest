...
removed padding and budgets height for the *expanded* timeline so the page
never scrolls; the expanded chart height dropped to `h-28` so the board doesn't
collapse when the band opens.
...
`localStorage` (`blunderfest.timelineActiveLayer`); expand/collapse persists as
before (`blunderfest.timelineExpanded`). The old `blunderfest.timelineLayers`
and `blunderfest.timelineSpotlight` keys are retired. The analyze action, its
...
- The timeline is dramatically simpler: one state (active layer + expanded),
  one rendering path, one persisted choice. The per-layer data and chart
  components are unchanged — only the band's switching/visibility logic went.
...
- The board is smaller when the band is expanded (the height budget reserves
  the expanded strip). Collapsing the band returns the space without moving
  the board — the board size no longer jumps between states.
